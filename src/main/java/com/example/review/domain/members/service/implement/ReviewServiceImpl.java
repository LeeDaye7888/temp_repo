...
import com.example.review.domain.members.repository.ReviewRepository;
...
public class ReviewServiceImpl implements ReviewService {
    ...
    public ReviewPageResponse getAllByMember(User user, Pageable pageable) {
        ...
        PageRequest pageRequest = PageRequest.of(pageable.getPageNumber(), pageable.getPageSize(),
            pageable.getSort());
        Page<Review> reviews = reviewRepository.findAllByMember(member,
            pageRequest); // pageRequest가 pageable의 구현체 중 하나여서 가능햇던 것 같음 더 찾아보기
        List<ReviewResponse> reviewsList = reviews.getContent().stream()
            .map(review -> new ReviewResponse(review.getReviewId(),
                review.getReviewTitle(),
                review.getReviewContent(),
